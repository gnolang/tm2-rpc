...
The release workflow publishes with [npm trusted publishing](https://docs.npmjs.com/trusted-publishers) (GitHub OIDC) and provenance, so no npm token secret is needed. `@gnolang/tm2-rpc` must have `gnolang/tm2-rpc` with the `release.yaml` workflow configured as a trusted publisher on npmjs.com. `GITHUB_TOKEN` is provided automatically.

### Pre-changesets history

Versions `0.0.1` through `1.0.0` predate this workflow and were released manually. From the next release onward, the changelog is generated by Changesets.
